...
- `runtime/agent/runtime/runtime_options.go` is either deleted or reduced to only `RunOption` setters (`WithRunID`, `WithLabels`, `WithTurnID`, `WithMetadata`, `WithTaskQueue`, `WithMemo`, `WithSearchAttributes`, `WithWorkflowOptions`, `WithTiming`, `WithPerTurnMaxToolCalls`, `WithRunMaxToolCalls`, `WithRunMaxRecoveryTurns`, `WithRunTimeBudget`, `WithRunFinalizerGrace`, `WithRunInterruptsAllowed`, `WithRestrictToTool`, `WithAllowedTags`, `WithDeniedTags`) and the `MissingFieldsAction` type / `defaultRetriedActivityPolicy` helper.
