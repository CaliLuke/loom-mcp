...
const (
	genAIPartTypeKey        = "type"
	genAIDocumentModality   = "document"
	genAIUnknownPartType    = "unknown"
	genAITextPartType       = "text"
	genAIBlobPartType       = "blob"
	genAIURIType            = "uri"
	genAIToolCallPartType   = "tool_call"
	genAIToolResultPartType = "tool_call_response"
)

// GenAIInputMessagesAttr serializes input messages into the OpenTelemetry GenAI
// message schema. Message content can be sensitive, so callers must explicitly
// opt in before attaching this attribute. Reasoning content is never captured.
func GenAIInputMessagesAttr(messages []*model.Message) (attribute.KeyValue, bool, error) {
	...
	for _, msg := range messages {
		...
		out = append(out, genAIInputMessage{Role: msg.Role, Parts: parts})
	}
	if len(out) == 0 {
		return attribute.KeyValue{}, false, nil
	}
...
// GenAIOutputMessagesAttr serializes output messages into the OpenTelemetry
// GenAI message schema. Message content can be sensitive, so callers must
// explicitly opt in before attaching this attribute. Reasoning content is never
// captured.
...
func genAIPart(part model.Part) (any, bool) {
	...
	case model.TextPart:
		...
		return genAITextPart{Type: genAITextPartType, Content: v.Text}, true
	case model.ToolUsePart:
		return genAIToolCallPart{Type: genAIToolCallPartType, ID: v.ID, Name: v.Name, Arguments: v.Input}, true
	case model.ToolResultPart:
		return genAIToolCallResponsePart{Type: genAIToolResultPartType, ID: v.ToolUseID, Response: v.Content}, true
	case model.ThinkingPart:
		return nil, false
	case model.ImagePart:
		return genAIBlobPart{
			Type:     genAIBlobPartType,
			Modality: "image",
	...
	case model.CitationsPart:
		return map[string]any{
			genAIPartTypeKey: "citations",
			"content":        v.Text,
			"citations":      v.Citations,
		}, true
	case model.CacheCheckpointPart:
		return map[string]any{genAIPartTypeKey: "cache_checkpoint"}, true
	default:
...
func genAIDocumentPart(part model.DocumentPart) any {
	...
	switch {
	case part.URI != "":
		return genAIURIPart{Type: genAIURIType, Modality: genAIDocumentModality, MIMEType: mimeType, URI: part.URI}
	case len(part.Bytes) > 0:
		return genAIBlobPart{
			Type:     genAIBlobPartType,
			Modality: genAIDocumentModality,
			MIMEType: mimeType,
	...
	case len(part.Chunks) > 0:
		...
		return out
	default:
		return map[string]any{genAIPartTypeKey: genAIUnknownPartType, "modality": genAIDocumentModality}
	}
}

func genAIDocumentTextPart(part model.DocumentPart, content string, mimeType string) map[string]any {
	out := map[string]any{
		genAIPartTypeKey: genAITextPartType,
		"modality":       genAIDocumentModality,
		"content":        content,
	}
	if part.Name != "" {
