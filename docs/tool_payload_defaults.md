...
Loom MCP generates **typed tool payload structs**, **JSON schemas**, and **codecs** from your
...
This is implemented to match the shared Loom transport contract: decode into a helper shape, then
transform into the final payload type with defaults applied.

## Summary

- **Decode JSON into a helper type** with pointer fields (“decode‑body” shape) so the codec can
  distinguish **missing** from **zero**.
- **Transform helper → final payload** using the shared `codegen.GoTransform` helper.
...
The pointer/value decision for primitive fields depends on a `useDefault` flag:
...
In loom-mcp, we treat **tool payloads as request-like**, so payload struct generation uses
`useDefault=true`.
...
- `codegen.GoTransform` emits code that:
...
This matches the transport generators: validation happens on the decode-body helper, then
...
If you mismatch them, the transform generator will treat value fields as pointers (or vice versa)
