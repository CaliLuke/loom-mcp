...
Arrays and maps of named objects are supported in both tool payloads and results.
Generated codecs convert these collections between public types and the private
transport package. Nested shared types keep their `struct:pkg:path` package location.
...
| `CompressAtTurns(n)`              | Inside `History`   | Summarize once N logical turns accumulate    |
| `CompressAtMaxInputTokens(n)`     | Inside `History`   | Summarize once exact input tokens exceed N   |
| `KeepMaxTurns(n)`                 | Inside `History`   | Keep at most N newest turns exact            |
| `KeepMaxInputTokens(n)`           | Inside `History`   | Keep newest whole turns within token budget  |
...
| `SkillDirectory(root)`                        | Option for `MCP`, or inside `Service`        | Exposes child skill directories as `skill://` resources. Skill IDs from frontmatter `id` or directory names must match `[A-Za-z0-9._-]+`                         |
...
| `DynamicPrompt(name, description, opts...)`   | Inside `Method` (with MCP enabled)           | Marks method as dynamic prompt generator                                                                                          |
...
| `AuthorizationServer(url)`                    | Option for `OAuth`                           | Advertises one HTTPS OAuth 2.0 issuer URL without a query or fragment; blank, relative, HTTP, query-bearing, and fragment-bearing URLs fail design validation |
...
Names matching `runtime.ToolCallMeta` fields (`run_id`, `session_id`,
`turn_id`, `tool_call_id`, `parent_tool_call_id`) are metadata-backed. Other
required string fields are label-backed and are read from run labels, for
...
Compression requires at least one trigger (`CompressAtTurns` or
`CompressAtMaxInputTokens`) and at least one retention budget (`KeepMaxTurns` or
`KeepMaxInputTokens`). Token budgets use exact provider token counts; models used
for token-budget compression must implement `model.TokenCounter`. When
compression is configured, generated agent config includes a `HistoryModel` field
that callers must supply with a `model.Client`. The runtime uses
`ModelClassSmall` for summarization.
