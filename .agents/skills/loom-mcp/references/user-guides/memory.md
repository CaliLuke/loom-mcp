...
Manage state with transcripts, memory stores, sessions, and runs in loom-mcp.

Memory & Sessions
=================

This guide covers loom-mcp’s transcript model, memory persistence, and how to model multi-turn conversations and long-running workflows.


Why Transcripts Matter
======================

loom-mcp treats the transcript as the single source of truth for a run: an ordered sequence of messages and tool interactions that is sufficient to:
...
You persist and pass the transcript only; loom-mcp and its provider adapters rebuild everything they need from that.


Messages and Parts
==================

At the model boundary, loom-mcp uses model.Message values to represent the transcript. Each message has a role (user, assistant) and an ordered list of parts:
...
loom-mcp’s provider adapters (e.g., Bedrock Converse) re-encode these parts into provider-specific blocks without reordering.


The Transcript Contract
=======================

The high-level transcript contract in loom-mcp is:
...
3. loom-mcp re-encodes that transcript into the provider’s chat format in the same order
...
loom-mcp separates conversation state into three layers:
...
loom-mcp’s feature modules provide complementary stores:
