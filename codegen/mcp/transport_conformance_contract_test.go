...
func TestGenerateTransportConformance(t *testing.T) {
	...
	t.Run("SDK owns sessions subscriptions progress and elicitation", func(t *testing.T) {
		...
		require.Contains(t, sdk, "mcpruntime.WithProgressToken(ctx, req.Params.GetProgressToken())")
		require.Contains(t, sdk, "sdkclient.WithClientFeatures(ctx, serverSession, sdkclient.ClientFeaturesOptions{")
		require.Contains(t, sdk, "sdkclient.InputRequired(err)")
		require.Contains(t, sdk, "RequestStateKey")
		require.Contains(t, sdk, "RequestMethod:")
		require.Contains(t, sdk, "RequestParams:")
