...
Remediation progress: **All exposed defects through C-72 are resolved; hosted run 29202009227 is fully green, and a completion audit is closing the remaining live zero-coverage owners in C-12.** Validation covers 49/59 DSL error sites and more than 37/46 `ToolExpr.Validate` conditions. Five materially different agent designs are freshly rendered and compiled. The compile matrix exposed and now prevents invalid cross-package payload conversion, required injected-pointer assignment, and result-less method invocation (C-16–C-18). A renewed package-level completeness pass raised the in-memory workflow engine from 37.8 % to 89.8 %, hook transport from 33.1 % to 87.6 %, SDK client-feature bridge from 4.5 % to 84.1 %, hint rendering from 38.4 % to 73.6 %, the in-memory session store from 50.7 % to 84.9 %, stream translation from 51.6 % to 79.3 %, the Temporal engine from 53.1 % to 75.2 %, durable transcript replay from 55.6 % to 78.7 %, the model wire/error boundary from 50.0 % to 91.7 %, telemetry from 45.7 % to 95.6 %, the registry tool executor from 50.0 % to 84.3 %, model middleware from 58.2 % to 83.2 %, durable memory event codecs from 62.6 % to 77.2 %, the MCP runtime from 64.5 % to 81.5 %, the Mongo runlog client from 59.8 % to 82.4 %, canonical tool-registry wire helpers from 0 % to 90.8 %, and structured tool errors from 0 % to 100 %, exposing thirty-five runtime defects (C-38–C-72). The final runlog slice found no additional product defect: its implementation held under the expanded fake and real-Mongo contracts.

## 1. Executive summary

The suite now contains 352 test files and 1,620 top-level test functions (plus one benchmark). Regression discipline remains strong: every audited fix has landed with a test, and remediation exposed fifty-eight additional product defects (C-15–C-72). Shuffle is enforced, design-validation targets are met, a five-design compile matrix rejects invalid generated package graphs, high-fan-in runtime and ordering boundaries are synchronized, all five provider adapters execute one behavioral conformance matrix, real Mongo contracts run through containers, and sampling, client roots, elicitation, and progress have both direct official-SDK adapter contracts and real client-vs-generated-framework contracts. Hosted Actions exposed C-36's host-dependent Markdown MIME classification and C-37's asynchronous replicated-map visibility, then passed both jobs after their fixes. The renewed completeness pass exposed and fixed in-memory workflow identity, cancellation, child-handle, and completed-signal defects; dropped hook display metadata and nil-input codec panics; optional-field, typed-count, and Unicode hint-rendering failures; cross-backend session ownership violations; incomplete/mutable client stream payloads; Temporal cancellation error drift; transcript replay corruption across message/part forms; asymmetric model-part serialization and token estimation; incorrect GenAI pointer-part export; nil/aliasing failures in registry-backed tool execution; clustered model-limiter policy drift; mutable durable-memory payload aliases; missing Temporal child run identity; canonical MCP decode panics/rejections; and lost structured tool-error context. The final runlog contract expansion found no further defect. The warm unit critical path is 27 % faster, duplication is resolved or reclassified, and protocol-safe scenario-file parallelism reduced the local integration critical path below 90 seconds. Integration fixture artifacts have process-level owners and are removed after every test binary; a measured run left zero prepared clones and zero cached server binaries after an earlier session had accumulated 93 clones (125 MB) and 218 binaries (8.2 GB).
...
| Test files / test cases | 352 files, 1,620 `func Test` plus 1 benchmark; latest full gate counts are refreshed after each remediation phase |
| Layer split (unit / integration / e2e) | 1,447 unit funcs; 172 integration-cluster funcs (framework 31, tests 7, assistant 125, agent_features 9); 1 e2e quickstart in the integration target |
| Total wall time (local) / (CI pipeline) | Unit (`-short -race -covermode=atomic`): **21.84 s fully warm** (59.05 s CPU), down from 30.1 s. Latest local scenarios: 72.1–75.4 s. Hosted run **29202009227 green**: integration 2m14s; strict build/lint/unit/coverage 7m42s. |
...
| Coverage (line / branch) | **68.8 %** statements in the latest strict local gate with Docker-backed packages through Colima; branch N/A (Go). Generated, mock, and fixture-only zero-coverage entries are attribution artifacts; four small live owners are explicitly tracked by reopened C-12 |
...
| Bug-fix commits with tests (sampled ratio) | **51/51** in the remediation delta (measured, not sampled); cumulative 66/66 |
...
### C-11 (positive): Regression discipline is excellent — now measured at 51/51 on the delta
- **Severity:** low (positive) · **Status:** reconfirmed, stronger
- **Confidence:** measured
- **Evidence:** Every remediation fix commit, including C-28's executable Gemini conformance case and the renewed runtime matrices through C-72, touches tests. Quality spot-reads: `63f3a22` asserts cap size, FIFO eviction order, and concurrent-writer safety; `91028a1` asserts timeout bounding with elapsed-time and detached-context assertions; `eb3911e` race-hardened 1000-iteration loop. Not echoes.
- **Recommendation:** Preserve the norm; none.

### C-12: Zero-coverage inventory distinguishes attribution artifacts from live owners
- **Severity:** low · **Status:** reopened by completion audit; remediation in progress
- **Confidence:** measured
- **Evidence:** `testutil` is 16.7 % because the executable provider harness is directly tested. Generated registry packages, registry/design, generated mocks, `codegen/testhelpers`, and codegen test scenarios are test-time/generated attribution artifacts. The earlier audit incorrectly omitted four live owners: the engine context helpers, stream bridge, Mongo runlog store façade, and concrete Pulse client. Canonical tool-registry wire helpers and structured tool errors were also live at 0 %; direct behavioral matrices now raise them to 90.8 % and 100 % and exposed C-72.
- **Recommendation:** Add direct contracts for the four remaining owners, including a real Redis round trip for the concrete Pulse client; keep generated/mock attribution entries out of product-gap counts.
...
- 1.6a Test roots & selection mapping — **resolved locally and hosted**: canonical targets select all 172 integration-cluster functions plus the quickstart.
...
- 2.2 Coverage — re-measured via repo's own strict flags (68.8 %); attribution artifacts separated from four remaining live owners (C-12); note `runtime/agent/runtime/runtime.go` and `planner/planner.go` legitimately contain few executable statements (facade files) — not attribution failures.
...
- 2.7 Regression discipline — **measured 51/51 on the full remediation delta** (C-11).
...
|---|---|---|---|---|---|
| 1 | **Completed:** repaired CI selection/toolchain and Docker fail-closed behavior; hosted run green | C-1, C-2 | complete | done | 172 integration funcs + quickstart + 29 registry tests are a real gate |
...
Each slice first ran targeted race/shuffle stress and then `make lint`, Colima-backed `make test`, `make itest`, and `make verify-mcp-local`. Every exposed defect landed with its regression test and an audit-ledger entry in the same commit. The five selected second-pass boundaries are behaviorally complete, but the later requirement-by-requirement completion audit disproved the blanket classification of every remaining zero: C-12 now tracks the four live owners separately from generated, mock, and fixture-only attribution entries.
...
| C-1 | CI contract inventory + uncached `make itest` | workflow used Go `1.25.x`, Loom `@latest`, selected only 6 integration functions, and allowed cached scenario passes | this change | all 172 integration funcs plus quickstart green locally and hosted |
...
- **CI-remediation verification:** workflow YAML parses; `make build`, `make lint`, shuffled race-enabled `make test`, uncached expanded `make itest`, and `make verify-mcp-local` are green. The latest local scenarios ran in 71.2 s. With Colima exported through `DOCKER_HOST` and `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, the complete strict unit gate passed at 68.8 % coverage; without a discoverable/mountable socket, it failed closed. Hosted run 29202009227 passed integration in 2m14s and strict build/lint/unit/coverage in 7m42s.
