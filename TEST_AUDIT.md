...
| Test files / test cases | 335 files, 1,542 `func Test` plus 1 benchmark; latest full gate counts are refreshed after each remediation phase |
| Layer split (unit / integration / e2e) | 1,369 unit funcs; 172 integration-cluster funcs (framework 31, tests 7, assistant 125, agent_features 9); 1 e2e quickstart in the integration target |
...
| Coverage (line / branch) | **65.5 %** statements in the latest strict local gate with Docker-backed packages through Colima; branch N/A (Go). Three zero-coverage package groups remain attribution artifacts (C-12) |
...
| Bug-fix commits with tests (sampled ratio) | **40/40** in the remediation delta (measured, not sampled); cumulative 55/55 |
...
### C-11 (positive): Regression discipline is excellent — now measured at 40/40 on the delta
- **Severity:** low (positive) · **Status:** reconfirmed, stronger
- **Confidence:** measured
- **Evidence:** Every remediation fix commit, including C-28's executable Gemini conformance case, C-38–C-41's in-memory lifecycle contracts, C-42/C-43's full hook-codec matrix, C-44–C-46's user-visible hint rendering contract, and C-47–C-50's cross-backend session contracts, touches tests. Quality spot-reads: `63f3a22` asserts cap size, FIFO eviction order, and concurrent-writer safety; `91028a1` asserts timeout bounding with elapsed-time and detached-context assertions; `eb3911e` race-hardened 1000-iteration loop. Not echoes.
...
- 2.2 Coverage — re-measured via repo's own strict flags (65.5 %); attribution artifacts re-confirmed (C-12); note `runtime/agent/runtime/runtime.go` and `planner/planner.go` legitimately contain few executable statements (facade files) — not attribution failures.
...
- 2.7 Regression discipline — **measured 40/40 on the full remediation delta** (C-11).
...
- **CI-remediation verification:** workflow YAML parses; `make build`, `make lint`, shuffled race-enabled `make test`, uncached expanded `make itest`, and `make verify-mcp-local` are green. The latest local scenarios ran in 72.9 s. With Colima exported through `DOCKER_HOST` and `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, the complete strict unit gate passed at 65.5 % coverage; without a discoverable/mountable socket, it failed closed. Hosted run 29202009227 passed integration in 2m14s and strict build/lint/unit/coverage in 7m42s.
