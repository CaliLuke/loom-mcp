...
The suite now contains 325 test files and 1,512 top-level test functions. Regression discipline remains strong: every audited fix has landed with a test, and the remediation tests have already exposed eleven additional product defects (C-15–C-25). Shuffle is enforced, design-validation targets are met, a five-design compile matrix rejects invalid generated package graphs, high-fan-in runtime boundaries are directly exercised, provider error/stream termination gaps are closed, real Mongo contracts run through Colima, and sampling plus client roots now have real client-vs-generated-framework contracts. The CI configuration repair is implemented locally: its toolchain matches the modules, repository make targets own the commands, `make itest` reaches all integration clusters, and Docker-backed registry coverage fails closed in CI. **An actual green Actions run remains the immediate blocker** because these commits have not yet produced a hosted run. Progress and throughput findings remain open. Integration fixture artifacts now have process-level owners and are removed after every test binary; a measured run left zero prepared clones and zero cached server binaries after an earlier audit session had accumulated 93 clones (125 MB) and 218 binaries (8.2 GB).
...
| Test files / test cases | 325 files, 1,512 `func Test`; latest full gate counts are refreshed after each remediation phase |
...
### C-5: MCP spec gaps: sampling and roots covered; progress remains
- **Severity:** medium · **Status:** partially resolved
- **Confidence:** measured
- **Evidence:** `TestGeneratedServerSDKToolCanSampleThroughClient` drives a generated `sample_text` tool through the official Go SDK over real streamable HTTP. The client advertises sampling through `CreateMessageHandler`; the server sends `sampling/createMessage` with role, text, system prompt, and token limit; the client response returns as structured tool output. That red/green cycle exposed C-24's nested-response flush deadlock. `TestGeneratedServerSDKToolCanListClientRoots` now drives a generated tool through `roots/list` and preserves root URI/name in structured output. `TestGeneratedSDKServerReceivesClientRootsListChanged` connects the official client and generated server over SDK transports, mutates the client's roots, and proves `notifications/roots/list_changed` reaches the configured server handler. Progress is the only original zero-coverage surface left.
- **Impact/Recommendation:** Sampling and roots are now supported runtime contracts with real client validation. Continue the official-SDK red/green workflow for progress-token propagation and monotonic `notifications/progress`; add raw wire assertions because progress metadata is easy for typed SDK helpers to normalize.
