...
| Test files / test cases | 328 files, 1,515 `func Test` plus 1 benchmark; latest full gate counts are refreshed after each remediation phase |
| Layer split (unit / integration / e2e) | 1,342 unit funcs; 172 integration-cluster funcs (framework 31, tests 7, assistant 125, agent_features 9); 1 e2e quickstart in the integration target |
| Total wall time (local) / (CI pipeline) | Unit (`-short -race -covermode=atomic`): **22.5 s fully warm** (59.7 s CPU), down from 30.1 s; first post-edit build was 41.8 s. Latest expanded `make itest`: quickstart 8.4 s, framework 19.3 s, scenarios 182.2 s (critical path), all green. CI: **never executed** (`gh api …/actions/runs` → `total_count: 0`) |
...
| Bug-fix commits with tests (sampled ratio) | **32/32** in the remediation delta (measured, not sampled); cumulative 47/47 |
...
- **Severity:** medium · **Status:** executable shared matrix implemented; 2/5 adapters migrated
- **Confidence:** measured
- **Evidence:** `testutil.RunProviderConformance` requires ordinary and rate-limit errors, malformed tool calls, cancellation, structured-output/tool-choice compatibility, usage accounting, and either the complete stream setup/receive/terminal lifecycle or one explicit unsupported-streaming contract. Gemini executes the unsupported-streaming form and exposed C-28. Ollama executes the full HTTP/NDJSON lifecycle and exposed C-29 (429 lacked the rate-limit sentinel) plus C-30 (completion and stream usage dropped model class).
- **Impact:** Provider expectations are executable rather than a prose checklist, and unsupported capabilities remain explicit instead of becoming missing callbacks. Anthropic, Bedrock, and OpenAI still need migration before drift is impossible.
- **Recommendation:** Migrate the three remaining adapters using their existing SDK-native fixtures. Treat every newly failing shared case as a product defect; do not move SDK-specific event construction into the shared harness.
...
### C-11 (positive): Regression discipline is excellent — now measured at 32/32 on the delta
...
- 2.7 Regression discipline — **measured 32/32 on the full remediation delta** (C-11).
...
- **CI-remediation verification:** `actionlint` green; `make lint`, shuffled race-enabled `make test`, uncached expanded `make itest`, and `make verify-mcp-local` green. The latest scenario package ran in 182.2 s; quickstart and framework passed in 8.4 s and 19.3 s. With Colima's socket exported through `DOCKER_HOST` and `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, the strict 29-test registry package passed under race/shuffle in 71.8 s; without a discoverable socket, the strict switch failed closed as designed.
