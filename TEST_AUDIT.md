...
| Test files / test cases | 344 files, 1,593 `func Test` plus 1 benchmark; latest full gate counts are refreshed after each remediation phase |
| Layer split (unit / integration / e2e) | 1,420 unit funcs; 172 integration-cluster funcs (framework 31, tests 7, assistant 125, agent_features 9); 1 e2e quickstart in the integration target |
...
| Coverage (line / branch) | **67.5 %** statements in the latest strict local gate with Docker-backed packages through Colima; branch N/A (Go). Three zero-coverage package groups remain attribution artifacts (C-12) |
...
| Bug-fix commits with tests (sampled ratio) | **48/48** in the remediation delta (measured, not sampled); cumulative 63/63 |
...
- 2.2 Coverage — re-measured via repo's own strict flags (67.5 %); attribution artifacts re-confirmed (C-12); note `runtime/agent/runtime/runtime.go` and `planner/planner.go` legitimately contain few executable statements (facade files) — not attribution failures.
...
- 2.7 Regression discipline — **measured 48/48 on the full remediation delta** (C-11).
...
- **CI-remediation verification:** workflow YAML parses; `make build`, `make lint`, shuffled race-enabled `make test`, uncached expanded `make itest`, and `make verify-mcp-local` are green. The latest local scenarios ran in 71.1 s. With Colima exported through `DOCKER_HOST` and `TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE`, the complete strict unit gate passed at 67.5 % coverage; without a discoverable/mountable socket, it failed closed. Hosted run 29202009227 passed integration in 2m14s and strict build/lint/unit/coverage in 7m42s.
