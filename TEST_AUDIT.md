...
The suite now contains 323 test files and 1,507 top-level test functions. Regression discipline remains strong: every audited fix has landed with a test, and the remediation tests have already exposed ten additional product defects (C-15–C-24). Shuffle is enforced, design-validation targets are met, a five-design compile matrix rejects invalid generated package graphs, high-fan-in runtime boundaries are directly exercised, provider error/stream termination gaps are closed, real Mongo contracts run through Colima, and text sampling now has a real client-vs-generated-framework contract. The CI configuration repair is implemented locally: its toolchain matches the modules, repository make targets own the commands, `make itest` reaches all integration clusters, and Docker-backed registry coverage fails closed in CI. **An actual green Actions run remains the immediate blocker** because these commits have not yet produced a hosted run. Roots, progress, and throughput findings remain open. Integration fixture artifacts now have process-level owners and are removed after every test binary; a measured run left zero prepared clones and zero cached server binaries after an earlier audit session had accumulated 93 clones (125 MB) and 218 binaries (8.2 GB).
...
| Test files / test cases | 323 files, 1,507 `func Test`; latest full gate counts are refreshed after each remediation phase |
...
### C-5: MCP spec gaps: sampling now covered; roots and progress remain
- **Severity:** medium · **Status:** partially resolved
- **Confidence:** measured
- **Evidence:** `TestGeneratedServerSDKToolCanSampleThroughClient` now drives a generated `sample_text` tool through the official Go SDK over real streamable HTTP. The client advertises sampling through `CreateMessageHandler`; the server sends `sampling/createMessage` with role, text, system prompt, and token limit; the client response returns as structured tool output. The first red run reported an unknown tool. After the sampling bridge existed, the test timed out and exposed C-24: the generated telemetry response writer hid flushing from `http.ResponseController`, deadlocking nested server-to-client messages. Both defects are fixed. Roots and progress still have no real client-vs-framework validation.
- **Impact/Recommendation:** Sampling is covered and text sampling is now a supported runtime contract. Continue the same official-SDK red/green workflow for roots/list plus roots/list_changed, then progress-token propagation and notifications; add raw wire assertions where SDK types normalize fields.
