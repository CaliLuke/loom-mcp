...
var (
	...
	// loomHeaderVersionRE matches Loom's generated header with or without version.
	loomHeaderVersionRE = regexp.MustCompile(`(?m)^// Code generated by Loom(?: v[^,]+)?, DO NOT EDIT\.`)

	// protoHeaderVersionRE matches Loom's generated header for .proto files.
	protoHeaderVersionRE = regexp.MustCompile(`(?m)^// Code generated with goa v[^,]+, DO NOT EDIT\.`)

	// goCommandRE normalizes generated command lines in Go source headers so
	// local invocation details do not churn goldens.
	goCommandRE = regexp.MustCompile(`(?m)^// Command:\n(?:(?://.*)\n)+`)
)
...
func normalizeGoHeader(b []byte) []byte {
	b = loomHeaderVersionRE.ReplaceAll(b, []byte("// Code generated by Loom vX, DO NOT EDIT."))
	return goCommandRE.ReplaceAll(b, []byte("// Command:\n"))
}
