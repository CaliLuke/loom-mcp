...
* **Service `assistant`:**
    * **Agent `assistant_runtime`** (ID: `assistant.assistant_runtime`):
        * **Mission:** *Runtime agent that owns projected tools*
        * **Uses Toolsets:**
            * `assistant.projected`
...
import (
    ...
    assistant_runtime "example.com/assistant/gen/assistant/agents/assistant_runtime"
)
...
func main() {
    ...
    {
        cfg := assistant_runtime.AssistantRuntimeAgentConfig{
            Planner: &StubPlanner{},
            // We'll add tool configurations here later on.
        }
        if err := assistant_runtime.RegisterAssistantRuntimeAgent(context.Background(), rt, cfg); err != nil {
            panic(err)
        }
    }
    ...
    client := assistant_runtime.NewClient(rt)
    out, err := client.Run(
        context.Background(),
        "my-first-session",
        []*model.Message{
...
<details>
<summary><strong>Agent: <code>assistant_runtime</code></strong> (ID: <code>assistant.assistant_runtime</code>)</summary>

* **Package:** `example.com/assistant/gen/assistant/agents/assistant_runtime`
* **Directory:** `gen/assistant/agents/assistant_runtime`
* **Config Struct:** `AssistantRuntimeAgentConfig`
* **Register Function:** `RegisterAssistantRuntimeAgent(ctx, rt, cfg)`
* **How to Run:**
    ...
    * **Asynchronous (get a handle):**
        ...
        ```
* **Workflow Name:** `assistant.assistant_runtime.workflow` (Queue: `assistant_assistant_runtime_workflow`)
...
### Agent `assistant_runtime` Toolsets
...
* *Return projected runtime status*
