...
func NewEndpoints(scheme string, host string, doer goahttp.Doer, enc func(*http.Request) goahttp.Encoder, dec func(*http.Response) goahttp.Decoder, restore bool) *assistant.Endpoints {
	...
	// Tool: search_records -> SearchRecords
	e.SearchRecords = func(ctx context.Context, v any) (any, error) {
		var payload any
		payload = v.(*assistant.SearchRecordsPayload)
		args, err := encodeOriginalPayload(ctx, enc, payload)
		if err != nil {
			return nil, err
		}
		...
		req3, err := origC.BuildSearchRecordsRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		decode := assistantjsonrpcc.DecodeSearchRecordsResponse(dec, false)
		return decodeOriginalJSONRPCResult(enc, req3, toolResp.Result, decode)
	}
	...
	e.ReportProgress = func(ctx context.Context, v any) (any, error) {
		...
		if err != nil {
			prompt := retry.BuildRepairPrompt("tools/call:report_progress", err.Error(), "{}", "")
			return nil, &retry.RetryableError{
				Cause:  err,
				Prompt: prompt,
			}
		...
		if len(toolResp.Result) == 0 {
			prompt := retry.BuildRepairPrompt("tools/call:report_progress", "empty MCP tool response", "{}", "")
			return nil, &retry.RetryableError{
				Cause:  fmt.Errorf("empty MCP tool response for report_progress"),
				Prompt: prompt,
			}
		}
		req3, err := origC.BuildReportProgressRequest(ctx, v)
		if err != nil {
			return nil, err
		}
		decode := assistantjsonrpcc.DecodeReportProgressResponse(dec, false)
		return decodeOriginalJSONRPCResult(enc, req3, toolResp.Result, decode)
	}
