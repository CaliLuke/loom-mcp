...
type Server struct {
	...
	// ElicitationContext is the handler for the elicitation_context method.
	ElicitationContext func(context.Context, *http.Request, *jsonrpc.RawRequest, http.ResponseWriter) error
...
func New(endpoints *assistant.Endpoints, mux loomhttp.Muxer, decoder func(*http.Request) loomhttp.Decoder, encoder func(context.Context, http.ResponseWriter) loomhttp.Encoder, errhandler func(context.Context, http.ResponseWriter, error), corsPolicy loomhttp.RuntimeCORSPolicy) *Server {
	s := &Server{
		...
		ElicitationContext:             NewElicitationContextHandler(endpoints.ElicitationContext, mux, decoder, encoder, errhandler),
...
func (s *Server) processRequest(ctx context.Context, r *http.Request, req *jsonrpc.RawRequest, w http.ResponseWriter) {
	...
	case "elicitation_context":
		if err := s.ElicitationContext(ctx, r, req, w); err != nil {
			loomtransport.RequestObserverFromContext(ctx).Fail(loomtransport.ReasonHandlerError)
			s.errhandler(ctx, w, fmt.Errorf("handler error for elicitation_context: %w", err))
		}
...
// NewElicitationContextHandler creates a JSON-RPC handler which calls the
// "assistant" service "elicitation_context" endpoint.
func NewElicitationContextHandler(endpoint loom.Endpoint, mux loomhttp.Muxer, decoder func(*http.Request) loomhttp.Decoder, encoder func(context.Context, http.ResponseWriter) loomhttp.Encoder, errhandler func(context.Context, http.ResponseWriter, error)) func(ctx context.Context, r *http.Request, req *jsonrpc.RawRequest, w http.ResponseWriter) error {
	return func(ctx context.Context, r *http.Request, req *jsonrpc.RawRequest, w http.ResponseWriter) error {
		ctx = context.WithValue(ctx, loom.MethodKey, "elicitation_context")
		ctx = context.WithValue(ctx, loom.ServiceKey, "assistant")
		...
		// Convert result to response body with proper JSON tags
		body := NewElicitationContextResponseBody(res.(*assistant.ElicitationContextResult))
		response := jsonrpc.MakeSuccessResponse(id, body)
		if err := encoder(ctx, w).Encode(response); err != nil {
			loomtransport.RequestObserverFromContext(ctx).Fail(loomtransport.ReasonResponseWriteFailed)
			errhandler(ctx, w, fmt.Errorf("failed to encode JSON-RPC response: %w", err))
