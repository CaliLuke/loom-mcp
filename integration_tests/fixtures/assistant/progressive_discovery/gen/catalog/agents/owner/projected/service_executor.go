...
//   exec := projected.NewOwnerProjectedExec(client)
//
//   // Register:
//   // reg := OwnerProjected.NewOwnerProjectedToolsetRegistration(exec)
...
func WithClient(client *catalog.Client) ExecOpt {
	return execOptFunc(func(c *seCfg) {
		if client == nil {
			return
		}
		...
		c.callers[tools.Ident("projected.projected_lookup")] = func(ctx context.Context, args any) (any, error) {
			return client.ProjectedLookup(ctx, args.(*catalog.ProjectedLookupPayload))
		}
	})
}

// WithProjectedLookup sets the caller for projected.projected_lookup.
func WithProjectedLookup(f func(context.Context, any) (any, error)) ExecOpt {
	return execOptFunc(func(c *seCfg) {
		if c.callers == nil {
			c.callers = make(map[tools.Ident]func(context.Context, any) (any, error))
		}
		c.callers[tools.Ident("projected.projected_lookup")] = f
	})
}

// NewOwnerProjectedExec returns a ToolCallExecutor that
...
func NewOwnerProjectedExec(opts ...ExecOpt) runtime.ToolCallExecutor {
	var cfg seCfg
	...
	{
		var missing []string
		if cfg.callers == nil || cfg.callers[tools.Ident("projected.projected_lookup")] == nil {
			// report the fully-qualified tool for clarity
			missing = append(missing, "projected.projected_lookup")
		}
		if len(missing) > 0 {
	...
	return runtime.ToolCallExecutorFunc(func(ctx context.Context, meta *runtime.ToolCallMeta, call *planner.ToolRequest) (*runtime.ToolExecutionResult, error) {
		...
		default:
			return runtime.Executed(&planner.ToolResult{
				Name: call.Name,
				Error: planner.NewToolError(
					fmt.Sprintf("tool %q is not a method-backed tool in toolset %q", call.Name, "catalog.projected"),
				),
			}), nil
		}
	})
