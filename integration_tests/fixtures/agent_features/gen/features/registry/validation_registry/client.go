...
//
// ValidationRegistry registry client
//
// Command:
...
// Client is a typed client for the validation-registry registry.
type Client struct {
	endpoint   string
...
// Static URL path constants for the validation-registry registry.
...
func NewClient(opts ...Option) *Client {
	c := &Client{
		endpoint:   "https://registry.fixture.invalid",
...
func (c *Client) GetToolset(ctx context.Context, name string) (*ToolsetSchema, error) {
	u := c.endpoint + pathToolsets + "/" + url.PathEscape(name)
...
func (c *Client) doRequest(ctx context.Context, method string, reqURL string, body []byte, result any) error {
	var lastErr error
	for attempt := 0; attempt <= c.retryMax; attempt++ {
		...
		lastErr = err
		// Don't retry on context cancellation or deterministic client errors
		if ctx.Err() != nil {
			return ctx.Err()
		}
		var regErr *RegistryError
		if errors.As(err, &regErr) && regErr.StatusCode >= http.StatusBadRequest && regErr.StatusCode < http.StatusInternalServerError && regErr.StatusCode != http.StatusTooManyRequests {
			return err
		}
	}
	return lastErr
}
func (c *Client) doSingleRequest(ctx context.Context, method string, reqURL string, body []byte, result any) error {
