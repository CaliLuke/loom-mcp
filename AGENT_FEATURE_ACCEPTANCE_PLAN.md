...
- 2026-07-06 — Milestone 6 restored remote Loom mode and passed `make verify-mcp-local`, `make lint`, `make test`, and `make itest`.
- 2026-07-06 — Review follow-up applied. Generated workflow retry-and-reflect is now proven through the default coordinator graph, debug endpoint assertions check concrete emitted await/memory/artifact/workflow state, stale checklist items were marked complete, and `PROGRESSIVE_TOOL_DISCOVERY_PLAN.*` artifacts were deleted instead of kept as sidecar docs.
...
- [x] In `TestGeneratedFeatureRunPersistsArtifactsMemorySkillsAndDebugState`, use the same generated runtime setup to execute a run whose `workflow.publish` returns an artifact, then assert artifact list/load, memory current-run/indexed lookup, skill list/load, and debug endpoints read emitted state.
- [x] In `TestGeneratedFeatureRunAppliesNamedInterceptorsAndRetryReflect`, register runtime-level interceptors with `agentsruntime.WithInterceptors`, register named `audit` with `agentsruntime.WithNamedInterceptors`, make `workflow.retry` fail once, and assert interceptor order plus planner-visible `RetryHint`.
