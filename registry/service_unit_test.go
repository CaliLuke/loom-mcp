...
import (
	...
	genregistrypb "github.com/CaliLuke/loom-mcp/v2/registry/gen/grpc/registry/pb"
	genregistryserver "github.com/CaliLuke/loom-mcp/v2/registry/gen/grpc/registry/server"
...
func TestCallToolEnsuresAdmissionWithActiveRegistrationToken(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, admissions.initialized)
...
func TestCompleteToolCallRejectsAggregateOversizeBeforeDependencies(t *testing.T) {
	t.Parallel()

	err := (&Service{}).CompleteToolCall(
		context.Background(),
		&genregistry.CompleteToolCallPayload{
			ResultJSON: []byte(strings.Repeat(" ", toolregistry.MaxToolResultMessageBytes+1)),
		},
	)
	require.Error(t, err)
	var serviceErr *goa.ServiceError
