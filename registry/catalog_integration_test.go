...
func TestAuthoritativeCatalogKeysAreSorted(t *testing.T) {
	ctx := context.Background()
	name := fmt.Sprintf("catalog-key-order-%d", time.Now().UnixNano())
	m, err := rmap.Join(ctx, name, testRedisClient)
	require.NoError(t, err)
	t.Cleanup(m.Close)

	for _, key := range []string{"registry:toolset:zeta", "registry:toolset:alpha", "registry:toolset:middle"} {
		inserted, insertErr := m.SetIfNotExists(ctx, key, key)
		require.NoError(t, insertErr)
		require.True(t, inserted)
	}

	keys, err := (authoritativeCatalogMap{Map: m, rdb: testRedisClient}).AuthoritativeKeys(ctx)
	require.NoError(t, err)
	assert.Equal(t, []string{
		"registry:toolset:alpha",
		"registry:toolset:middle",
		"registry:toolset:zeta",
	}, keys)
}

func TestRedisCatalogTombstonesPreventAToBToAResurrection(t *testing.T) {
	ctx := context.Background()
	name := fmt.Sprintf("catalog-resurrection-%d", time.Now().UnixNano())
	mapA, err := rmap.Join(ctx, name, testRedisClient)
	require.NoError(t, err)
	t.Cleanup(mapA.Close)
	mapB, err := rmap.Join(ctx, name, testRedisClient)
	require.NoError(t, err)
	t.Cleanup(mapB.Close)
	clock := newRedisTimeSource(testRedisClient)
	catalogA := newToolsetCatalog(authoritativeCatalogMap{Map: mapA, rdb: testRedisClient}, clock)
	catalogB := newToolsetCatalog(authoritativeCatalogMap{Map: mapB, rdb: testRedisClient}, clock)

	admissionA, err := catalogA.Register(
		ctx,
		testCatalogToolset("test.toolset", "A", nil),
		testAdmissionRevisionA,
		"provider-a",
		testIncarnationA,
		time.Minute,
	...
	require.NoError(t, err)
	require.NoError(t, catalogB.Retire(ctx, "test.toolset", admissionA.RegistrationToken))
	require.NoError(t, catalogB.ReleaseProvider(
		ctx,
		"test.toolset",
		"provider-a",
		testIncarnationA,
	...
	admissionB, err := catalogB.Register(
		ctx,
		testCatalogToolset("test.toolset", "B", nil),
		testAdmissionRevisionB,
		"provider-b",
		testIncarnationB,
		time.Minute,
	...
	require.NoError(t, catalogB.ReleaseProvider(
		...
		"provider-b",
		testIncarnationB,
		admissionB.RegistrationToken,
	))

	_, err = catalogA.Register(
		ctx,
		testCatalogToolset("test.toolset", "A", nil),
		testAdmissionRevisionA,
		"provider-a-returned",
	...
	admissionA2, err := catalogA.Register(
		ctx,
		testCatalogToolset("test.toolset", "A2", nil),
		"2026-07-23.3",
		"provider-a-new",
		testIncarnationA,
		time.Minute,
	...
	require.NoError(t, err)
	assert.NotEqual(t, admissionA.RegistrationToken, admissionA2.RegistrationToken)
	assert.Contains(t, admissionA2.RetiredTokens, admissionA.RegistrationToken)
	assert.Contains(t, admissionA2.RetiredTokens, admissionB.RegistrationToken)
}
...
func TestLiveRegistryRecoversOwnedStateLossSameName(t *testing.T) {
	...
	// Simulate Redis losing the toolset stream's state and consumer group rather
	// than an intentional Stream.Destroy: destruction leaves
