...
import (
	...
	"github.com/CaliLuke/loom-mcp/v2/runtime/toolregistry/provider"
...
func TestProviderRecoversAfterRedisStateLoss(t *testing.T) {
	...
	servedIncarnation := make(chan string, 1)
	var captureIncarnation sync.Once
	...
	go func() {
		registration := provider.Registration{
			...
			Register: func(ctx context.Context, _, _, incarnationID, _ string) (provider.RegistrationLease, error) {
				captureIncarnation.Do(func() {
					servedIncarnation <- incarnationID
				})
	...
	servedIncarnationID := <-servedIncarnation
	require.NotEqual(t, payload.ProviderIncarnationID, servedIncarnationID)
	cancel()
	require.ErrorIs(t, <-serveErr, context.Canceled)
	active, err := reg.service.catalog.ActiveRegistration(context.Background(), toolset)
	require.NoError(t, err)
	assert.NotContains(
		t,
		active.ProviderLeases,
		providerLeaseKey(payload.ProviderID, servedIncarnationID),
		"Serve must release its exact generated incarnation lease",
	)
}
