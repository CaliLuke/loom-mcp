...
func newService(opts serviceOptions) (*Service, error) { //nolint:maintidx // Constructor validates one complete registry dependency contract.
	if opts.catalog == nil {
...
func (s *Service) Register(ctx context.Context, p *genregistry.RegisterPayload) (*genregistry.RegisterResult, error) { //nolint:maintidx // Boundary mapping preserves every catalog admission outcome.
...
func (s *Service) CallTool(ctx context.Context, p *genregistry.CallToolPayload) (*genregistry.CallToolResult, error) { //nolint:maintidx // Initial admission, publication, and durable rejection form one service transaction.
	if err := toolregistry.ValidateWireProtocolVersion(p.WireProtocolVersion); err != nil {
		return nil, genregistry.MakeValidationError(err)
	}
...
func (s *Service) RetryTool(ctx context.Context, p *genregistry.RetryToolPayload) (*genregistry.CallToolResult, error) { //nolint:maintidx // Retry attachment preserves the original immutable admission in one boundary.
	if err := toolregistry.ValidateWireProtocolVersion(p.WireProtocolVersion); err != nil {
		return nil, genregistry.MakeValidationError(err)
	}
	prepared, err := prepareToolCallIdentity(
...
func (s *Service) CompleteToolCall(ctx context.Context, p *genregistry.CompleteToolCallPayload) error {
	if len(p.ResultJSON) > toolregistry.MaxToolResultMessageBytes {
		return genregistry.MakeValidationError(fmt.Errorf(
			"terminal result exceeds %d bytes",
			toolregistry.MaxToolResultMessageBytes,
		))
	}
...
func callRejectionFromError(err error) (callRejection, error) {
	...
	case string(callRejectionValidation):
