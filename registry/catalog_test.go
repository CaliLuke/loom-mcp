...
func TestCatalogReadAndDiscoveryAPIsUseActiveAdmission(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	now := time.Unix(1_700_000_000, 0)
	catalog := newToolsetCatalog(newTestCatalogMap(), newTestTimeSource(now))
	alpha, err := catalog.Register(
		ctx,
		testCatalogToolset("alpha.search", "Find release notes", []string{"search", "public"}),
	...
	require.NoError(t, err)
	beta, err := catalog.Register(
		ctx,
		testCatalogToolset("beta.audit", "Inspect history", []string{"audit", "private"}),
		testAdmissionRevisionA,
		"provider-beta",
		testIncarnationB,
		time.Minute,
	...
	toolset, err := catalog.GetToolset(ctx, "alpha.search")
	require.NoError(t, err)
	assert.Equal(t, "alpha.search", toolset.Name)
	_, err = catalog.GetToolset(ctx, "missing")
	require.ErrorIs(t, err, errToolsetNotFound)

	token, err := catalog.RegistrationToken(ctx, "alpha.search")
	require.NoError(t, err)
	assert.Equal(t, alpha.RegistrationToken, token)
	require.NoError(t, catalog.VerifyActiveToken(ctx, "alpha.search", token))
	require.ErrorIs(t, catalog.VerifyActiveToken(ctx, "alpha.search", testStaleToken), errToolsetNotFound)

	leases, err := catalog.ActiveProviderLeases(ctx, "alpha.search", token)
	require.NoError(t, err)
	require.Equal(t, []providerLeaseRecord{{
		ProviderID:            "provider-alpha",
		IncarnationID:         testIncarnationA,
		RegistrationToken:     token,
		LeaseExpiresUnixMilli: now.Add(time.Minute).UnixMilli(),
	}}, leases)
	_, err = catalog.ActiveProviderLeases(ctx, "alpha.search", testStaleToken)
	require.ErrorIs(t, err, errToolsetNotFound)

	listed, err := catalog.ListToolsets(ctx, []string{"public", "search"})
	require.NoError(t, err)
	require.Len(t, listed, 1)
	assert.Equal(t, "alpha.search", listed[0].Name)
	listed, err = catalog.ListToolsets(ctx, []string{"missing"})
	require.NoError(t, err)
	assert.Empty(t, listed)

	for query, want := range map[string]string{
		"ALPHA":   "alpha.search",
		"release": "alpha.search",
		"PRIVATE": "beta.audit",
	} {
		found, searchErr := catalog.SearchToolsets(ctx, query)
		require.NoError(t, searchErr)
		require.Len(t, found, 1)
		assert.Equal(t, want, found[0].Name)
	}

	require.NoError(t, catalog.Retire(ctx, "beta.audit", beta.RegistrationToken))
	found, err := catalog.SearchToolsets(ctx, "private")
	require.NoError(t, err)
	assert.Empty(t, found)

	canceled, cancel := context.WithCancel(ctx)
	cancel()
	_, err = catalog.ListToolsets(canceled, nil)
	require.ErrorIs(t, err, context.Canceled)
	_, err = catalog.SearchToolsets(canceled, "alpha")
	require.ErrorIs(t, err, context.Canceled)
}

func TestCatalogDiscoveryResultsAreSortedByName(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	m := newTestCatalogMap()
	m.authoritativeKeys = []string{
		toolsetCatalogKey("zeta.search"),
		toolsetCatalogKey("middle.search"),
		toolsetCatalogKey("alpha.search"),
	}
	catalog := newToolsetCatalog(
		m,
		newTestTimeSource(time.Unix(1_700_000_000, 0)),
	)
	for _, name := range []string{"zeta.search", "alpha.search", "middle.search"} {
		_, err := catalog.Register(
			ctx,
			testCatalogToolset(name, "Common searchable description", []string{"common"}),
			testAdmissionRevisionA,
			"provider-"+name,
			testIncarnationA,
			time.Minute,
	...
	listed, err := catalog.ListToolsets(ctx, []string{"common"})
	require.NoError(t, err)
	assert.Equal(t, []string{"alpha.search", "middle.search", "zeta.search"}, toolsetNames(listed))

	found, err := catalog.SearchToolsets(ctx, "common")
	require.NoError(t, err)
	assert.Equal(t, []string{"alpha.search", "middle.search", "zeta.search"}, toolsetNames(found))
}
...
func toolsetNames(toolsets []*genregistry.Toolset) []string {
...
func (m *testCatalogMap) AuthoritativeKeys(context.Context) ([]string, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if m.authoritativeKeys != nil {
		return append([]string(nil), m.authoritativeKeys...), nil
	}
	keys := make([]string, 0, len(m.content))
	for key := range m.content {
		keys = append(keys, key)
