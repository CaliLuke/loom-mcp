...
func TestCallToolReturnsReadableResultStream(t *testing.T) {
	...
	require.EqualValues(t, 1, rdb.Del(ctx, resultStreamKey).Val())
...
func TestLostDispatchLeaseCommitsAndRestoresOutcomeUnknown(t *testing.T) {
	...
	assert.Contains(t, terminal.Error.Message, "effect may have occurred")
...
func TestOutputDeltasAreCountBoundedAndPostTerminalSuppressed(t *testing.T) {
	...
	admission, err := svc.Register(ctx, provider)
	require.NoError(t, err)
	callPayload := transitionCallPayload(toolset, "deltas")
	call, err := svc.CallTool(ctx, callPayload)
	require.NoError(t, err)
	requireResultStreamExpiration(t, ctx, rdb, call)
	store := svc.callAdmissions.(*callAdmissionStore)
	...
	require.NoError(t, err)
	assert.Equal(t, call, replayed)
	requireResultStreamExpiration(t, ctx, rdb, replayed)

	callKey := store.callKey(call.ToolUseID)
	terminalEventID, err := rdb.HGet(ctx, callKey, "terminal_event_id").Result()
	require.NoError(t, err)
	require.EqualValues(t, 1, rdb.XDel(ctx, streamKey, terminalEventID).Val())
	restored, err := svc.CallTool(ctx, callPayload)
	require.NoError(t, err)
	assert.Equal(t, call, restored)
	requireResultStreamExpiration(t, ctx, rdb, restored)
}
...
func requireResultStreamExpiration(
	t *testing.T,
	ctx context.Context,
	rdb *redis.Client,
	call *genregistry.CallToolResult,
	...
	require.NoError(t, err)
	streamKey := pulseStreamKeyPrefix + toolregistry.ResultStreamID(call.ToolUseID)
	expiration, err := rdb.PExpireTime(ctx, streamKey).Result()
	require.NoError(t, err)
	require.Equal(t, expiresAt.UnixMilli(), int64(expiration/time.Millisecond))
}
